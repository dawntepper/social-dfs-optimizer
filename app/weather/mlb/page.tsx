...
export default function MLBWeatherPage() {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        const response = await fetch('/api/weather/mlb');
        if (!response.ok) {
    ...
    return (
      <div className="container mx-auto p-6">
        <Breadcrumb>
          ...
          <BreadcrumbItem>MLB</BreadcrumbItem>
        </Breadcrumb>
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="mt-6">
        <h1 className="text-3xl font-bold mb-6">MLB Ballpark Weather</h1>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <CardTitle className="flex items-center gap-2">
                        {stadium.stadium}
                        {stadium.isIndoors && (
                          <Badge variant="secondary">Retractable Roof</Badge>
                        )}
                      </CardTitle>
                      <p className="text-sm text-muted-foreground">Home of the {stadium.team}</p>
                    </div>
                    <Badge variant="outline">
                      {new Date(stadium.firstPitch).toLocaleTimeString([], { 
                        hour: 'numeric', 
                        minute: '2-digit'
                      })}
                    </Badge>
